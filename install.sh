...
PRODUCT=PressHarden
PROGRAM=pressharden
MODE="${PRESSHARDEN_INSTALL_MODE:-portable}"
case "$MODE" in local) MODE=portable;; legacy) MODE=user;; esac
case "$MODE" in portable) DEST="${PRESSHARDEN_INSTALL_PREFIX:-${PRESSHARDEN_INSTALL_DIR:-$PWD/PressHarden}}";; user) DEST="${PRESSHARDEN_INSTALL_PREFIX:-${PRESSHARDEN_PREFIX:-${XDG_DATA_HOME:-$HOME/.local/share}/pressharden}}";; *) echo 'Install mode must be portable or user.' >&2; exit 2;; esac
for dep in php tar mktemp find; do command -v "$dep" >/dev/null || { printf 'Missing dependency: %s\n' "$dep" >&2; exit 2; }; done
WORK=$(mktemp -d "${TMPDIR:-/tmp}/pressharden-install.XXXXXXXX")
STAGE=''
trap 'rm -rf -- "$WORK"; [ -z "$STAGE" ] || rm -rf -- "$STAGE"' EXIT
cat > "$WORK/guard.php" <<'PHP_GUARD'
<?php
/** PressHarden update input validation (MIT). Never executes archive contents. */
function ph_update_octal($field) {
    $field = trim($field, " \0");
    if ($field === '') return 0;
...
function ph_update_read($stream, $length) {
    $out = '';
...
function ph_update_archive($path) {
    ...
    try {
        while (true) {
            $header = ph_update_read($stream, 512); $total += 512;
            if ($header === $zero) {
                if (ph_update_read($stream, 512) !== $zero) throw new RuntimeException('invalid tar end marker');
                $total += 512;
            ...
            if ($sum !== ph_update_octal(substr($header, 148, 8))) throw new RuntimeException('invalid tar header checksum');
            $magic = substr($header, 257, 6);
            if ($magic !== "ustar\0" && $magic !== 'ustar ') throw new RuntimeException('unsupported tar format');
            $size = ph_update_octal(substr($header, 124, 12));
            $mode = ph_update_octal(substr($header, 100, 8));
            ...
            if ($type === 'g' || $type === 'x') {
                ...
                ph_update_pax(substr(ph_update_read($stream, $blocks), 0, $size));
                continue;
            }
            ...
            $seen[$name] = $type;
            while ($blocks > 0) { $read = min(65536, $blocks); ph_update_read($stream, $read); $blocks -= $read; }
        }
    } finally { gzclose($stream); }
}
function ph_update_canonical($path) {
    if ($path === '') return '';
...
if (isset($argv) && realpath($argv[0]) === __FILE__) {
    try {
        if (($argv[1] ?? '') === 'archive' && $argc === 3) ph_update_archive($argv[2]);
        elseif (($argv[1] ?? '') === 'layout' && $argc >= 5) ph_update_layout(array_slice($argv, 2));
...
PHP_GUARD
if [ -n "${PRESSHARDEN_INSTALL_SOURCE:-}" ]; then
  SRC=$(cd -P -- "$PRESSHARDEN_INSTALL_SOURCE" && pwd -P)
  ...
  curl -fsSL --proto '=https' --proto-redir '=https' --connect-timeout 15 --max-time 120 --max-filesize 33554432 \
    "https://github.com/marketania/PressHarden/archive/refs/heads/main.tar.gz" -o "$WORK/source.tar.gz"
...
php -r 'require $argv[1];ph_ops_path($argv[2]);' "$SRC/lib/ops-safety.php" "$DEST"
[ ! -e "$DEST" ] && [ ! -L "$DEST" ] || { echo 'Destination exists. Use its update command; installation will not overwrite it.' >&2; exit 2; }
PARENT=$(dirname "$DEST")
mkdir -p -- "$PARENT"
STAGE=$(mktemp -d "$PARENT/.pressharden-install.XXXXXXXX")
for item in .github .gitignore CHANGELOG.md CONTRIBUTING.md LICENSE README.md SECURITY.md VERSION PRODUCT PROVENANCE.md checks docs lib tests install.sh uninstall.sh "$PROGRAM"; do
...
if [ "$MODE" = portable ]; then
  : > "$STAGE/.pressharden-portable"
  cp "$SRC/config/config.example" "$STAGE/config/config"
else
  CFG="${PRESSHARDEN_CONFIG_FILE:-${XDG_CONFIG_HOME:-$HOME/.config}/pressharden/config}"
