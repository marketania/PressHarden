...
T=$(mktemp -d "${TMPDIR:-/tmp}/pressharden-discovery.XXXXXX")
trap 'rm -rf "$T"' EXIT
...
#!/usr/bin/env bash
set -u
marker="${PH_TEST_FIND_MARKER:-}"
[ -z "$marker" ] || : > "$marker"
/usr/bin/find "$@"
rc=$?
if [ "${PH_TEST_FAIL_DISCOVERY_FIND:-0}" = 1 ]; then
...
env_common=(ROOT="$ROOT" PRESSHARDEN_SCAN_ROOT="$ROOT" PRESSHARDEN_CONFIG_FILE="$T/no-config" PRESSHARDEN_STATE_DIR="$STATE" PRESSHARDEN_CACHE_DIR="$CACHE" PRESSHARDEN_DISCOVERY_CACHE_TTL=300 PRESSHARDEN_NOCOLOR=1)

# A traversal command that yields a valid site but exits nonzero is partial discovery, never clean.
set +e
out=$(env "${env_common[@]}" PATH="$T/bin:$PATH" PH_TEST_FAIL_DISCOVERY_FIND=1 bash -c '. "$1/lib/env-discovery.sh"; printf "%s|%s|%s\n" "$PH_DISCOVERY_FAILED" "${#SCAN_ROOTS[@]}" "${SCAN_ROOTS[0]:-}"' _ "$REPO" 2>&1)
rc=$?
set -e
...
CHECK
set +e
env "${env_common[@]}" PATH="$T/bin:$PATH" PH_TEST_FAIL_DISCOVERY_FIND=1 bash "$T/direct-check.sh" "$REPO" > "$T/direct.out" 2>&1
rc=$?
set -e
[ "$rc" -eq 2 ]; grep -q 'INCOMPLETE' "$T/direct.out"; ! grep -q '^  CLEAN' "$T/direct.out"

# Generate a good cache first.
env "${env_common[@]}" PRESSHARDEN_DISCOVERY_REFRESH=1 bash -c '. "$1/lib/env-discovery.sh"; [ "$PH_DISCOVERY_FAILED" -eq 0 ]; [ "${#SCAN_ROOTS[@]}" -eq 1 ]' _ "$REPO"
...
out=$(env "${env_common[@]}" PATH="$T/bin:$PATH" PH_TEST_FIND_MARKER="$marker" bash -c '. "$1/lib/env-discovery.sh"; printf "%s|%s\n" "$PH_DISCOVERY_FAILED" "${SCAN_ROOTS[0]:-}"' _ "$REPO")
