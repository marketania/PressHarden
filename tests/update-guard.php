...
$dir = sys_get_temp_dir().'/pressharden-guard-'.bin2hex(random_bytes(8));
...
function checkArchive($name, $data, $valid = false, $terminator = true) {
    ...
    $ok = true;
    try { ph_update_archive($path); } catch (RuntimeException $e) { $ok = false; }
...
try {
    ...
    $rejected = false; try { ph_update_archive($path); } catch (RuntimeException $e) { $rejected = strpos($e->getMessage(), '128 MiB') !== false; }
    if (!$rejected) throw new RuntimeException('expanded-byte cap failed'); ++$count;
    echo "PressHarden archive guard: $count cases PASS\n";
