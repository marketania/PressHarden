...
foreach(['EXAMPLE.com'=>'example.com','https://Example.com/'=>'example.com','example.com/Shop'=>'example.com/Shop','https://example.com/shop/'=>'example.com/shop','www.example.com'=>'www.example.com','example.com./stage'=>'example.com/stage'] as $a=>$b)check(ph_site_name($a)===$b,'normalization '.$a);
foreach(['example.com?x=1','example.com/#secret','example.com:443','user:pass@example.com','example.com/../foo','example.com/./foo','example.com//foo','example.com/*','-example.com','example..com','example.com/$(id)',"example.com\nx",'ftp://example.com','example','/tmp/site'] as $bad)check(ph_site_name($bad)===null,'invalid name');
foreach([
...
] as [$site,$scope,$want])check(ph_site_directory_name($site,$scope)===$want,'hosting layout');
