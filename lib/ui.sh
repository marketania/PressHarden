...
banner() {
  ...
  printf '%s%s  PRESSHARDEN OPERATIONS%s  %sv%s%s\n' "$B" "$C" "$X" "$D" "$PRESSHARDEN_VERSION" "$X"
  printf '  %s%-11s%s %s%s%s\n' "$D" "CHECK" "$X" "$B" "$NAME" "$X"
  if [ -n "${PH_SUITE_TOTAL:-}" ]; then _meta_field 11 "PROGRESS" "${PH_SUITE_PERCENT:-0}% suite complete • step ${PH_SUITE_CURRENT:-?}/${PH_SUITE_TOTAL}"; fi
  [ -z "${PH_REPORT_ID:-}" ] || _meta_field 11 "RUN" "$PH_REPORT_ID"
  _meta_field 11 "PURPOSE" "$DESC"; _meta_field 11 "CHECKS" "$SCAN_DOES"; _meta_field 11 "WHY" "$SCAN_WHY"
  printf '  %s%-11s%s %s\n' "$D" "ROOT" "$X" "$ROOT"
  [ "$PRESSHARDEN_CONFIG_LOADED" -eq 1 ] && printf '  %s%-11s%s %s\n' "$D" "CONFIG" "$X" "$PRESSHARDEN_CONFIG_FILE"
  printf '  %s%-11s%s %s%s%s WordPress install(s) across %s site group(s)\n' "$D" "SITES" "$X" "$B" "$(count_sites)" "$X" "$(count_domains)"
  [ "${PH_DISCOVERY_FAILED:-0}" -eq 0 ] || _meta_field 11 "COVERAGE" "INCOMPLETE discovery: validated sites are shown, but one or more paths could not be fully inspected."
