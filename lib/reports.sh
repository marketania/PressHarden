...
ph_report_init() {
  ...
  PH_REPORT_ID=${reservation##*/}; PH_REPORT_ID=${PH_REPORT_ID#.}
  PH_REPORT_PREFIX="$REPORTS/$PH_REPORT_ID"
  # Keep flat .log / *-findings.log / *-summary.json naming for existing users.
  for suffix in .log -findings.log -deletions.log; do
    file="$PH_REPORT_PREFIX$suffix"
  ...
  LOG="$PH_REPORT_PREFIX.log"
  DETAIL_LOG="$PH_REPORT_PREFIX-findings.log"
  DELETE_LOG="$PH_REPORT_PREFIX-deletions.log"
  PH_REPORT_FAILED=0
}
