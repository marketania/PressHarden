...
function ph_ops_dir($path) {
    $path = ph_ops_path($path);
...
function ph_ops_scope($state, array $sites) {
    if (!$sites) throw new RuntimeException('no selected WordPress sites');
    $state = ph_ops_path($state);
    foreach ($sites as $site) {
        $site = ph_ops_path($site);
    ...
    return ph_ops_dir($state);
}
function ph_ops_backup_dir($state, $site, $category) {
    if (!preg_match('/^[a-z][a-z0-9-]{0,50}$/D', $category)) throw new RuntimeException('invalid backup category');
    $state = ph_ops_scope($state, [$site]);
    $parent = ph_ops_dir($state.'/backups');
    $parent = ph_ops_dir($parent.'/'.$category);
...
function ph_ops_regular($file, $max = 4194304) {
    ph_ops_path(dirname($file)); $st = @lstat($file);
...
if (isset($argv[0]) && realpath($argv[0]) === __FILE__) {
    try {
        if (($argv[1] ?? '') === 'scope' && $argc >= 4) ph_ops_scope($argv[2], array_slice($argv,3));
        elseif (($argv[1] ?? '') === 'backup-dir' && $argc === 5) echo ph_ops_backup_dir($argv[2],$argv[3],$argv[4]),"\n";
        elseif (($argv[1] ?? '') === 'lock-file' && $argc === 4) echo ph_ops_lock_file($argv[2],$argv[3]),"\n";
        elseif (($argv[1] ?? '') === 'path' && $argc === 3) ph_ops_path($argv[2]);
        elseif (($argv[1] ?? '') === 'directory' && $argc === 3) echo ph_ops_dir($argv[2]),"\n";
