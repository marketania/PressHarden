...
function ph_site_directory_name($site, $scope) {
    ...
    if ($site === $scope && in_array(basename($site), ['public_html','httpdocs','htdocs','www','html'], true)) {
        return ph_site_name(basename(dirname($site)));
    }
    return null;
}

function ph_site_literal_names($site) {
    ...
    for ($i=0, $n=count($tokens); $i<$n; ++$i) {
        ...
        $name=ph_site_name($v); if ($name !== null) $names[$name]=true;
    }
    return array_keys($names);
}

function ph_site_index($paths, $scope, $aliases, $exclude) {
    ...
    foreach ($paths as $p) {
        ...
        $allowed[$p]=true;
        $name=ph_site_directory_name($p,$scope);
        $names=$name !== null ? [$name] : ph_site_literal_names($p);
        if (!$names) $unknown[]=$p;
    ...
    foreach (preg_split('~\r?\n~', $aliases) as $line) {
        ...
        $name=ph_site_name(trim($parts[0])); $p=trim($parts[1] ?? '');
...
if (isset($argv[0]) && realpath($argv[0]) === __FILE__) {
    try {
        ...
        $aliasFile=getenv('PRESSHARDEN_SITE_ALIASES_FILE')?:''; $aliases='';
        if($aliasFile!=='') {
        ...
        [$index,$unknown,$byName]=ph_site_index($paths,$argv[2],$aliases,getenv('PRESSHARDEN_EXCLUDE')?:'');
        ...
        if ($argv[1] === '--list') {
            ...
            $name=ph_site_name($argv[1]);
            if ($name === null || !isset($index[$name])) {
                fwrite(STDERR,"Website not found or excluded. Run ./pressharden sites to see local names; nothing was selected.\n"); exit(2);
            }
            $matches=array_keys($index[$name]);
            if (count($matches)!==1) {
                fwrite(STDERR,"Website name is ambiguous. Run ./pressharden sites and use an explicit directory; nothing was selected.\n"); exit(2);
            }
            $p=$matches[0]; $count=0;
